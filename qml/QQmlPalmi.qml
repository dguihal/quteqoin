...
Rectangle {
	...
	Rectangle {
		...
		border.color: postInput.color.hslLightness > 0.179 ? "black" : "white" // https://www.w3.org/TR/WCAG20/ contrast ratio with L1 = 0 and L1 = 255
	...
	*/
	...
	function switchBoard(boardName) {
		var oldBoardName = currentBoardName
		for (var i = 0; i <= boardListModel.count; i++) {
		...
		switchBoardNorloges(oldBoardName, boardName)
	}

	function switchBoardNorloges(oldBoard, newBoard) {
		var newTxt = ""
		var oldIdx = 0
		var refTxt = postInput.text

		var nRe = new RegExp(QQmlJS.norlogeExpStr, "g")
		var tabRes = []
		while ((tabRes = nRe.exec(refTxt)) !== null) {
			newTxt += refTxt.substring(oldIdx, tabRes.index)
			oldIdx = tabRes.index + tabRes[0].length

			var norlogeTxt = tabRes[0]
			if (tabRes[6] === '@' + newBoard)
				norlogeTxt = norlogeTxt.substring(
							0, norlogeTxt.length - tabRes[6].length)
			else if (typeof tabRes[6] === "undefined")
				norlogeTxt += "@" + oldBoard

			newTxt += norlogeTxt
		}
		newTxt += refTxt.substring(oldIdx, refTxt.length)

		postInput.text = newTxt
	}
